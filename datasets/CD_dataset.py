...
└─list
"""
IMG_FOLDER_NAME = "time1" # A
IMG_POST_FOLDER_NAME = 'time2' # B
...
def get_label_path(root_dir, img_name): # SYSU-CD is .png
...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        #print("self.img_name_list",self.img_name_list)
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        # print("printing A path", A_path)
        # print("printing B path", B_path)
        img = np.asarray(Image.open(A_path).convert('RGB'))
        img_B = np.asarray(Image.open(B_path).convert('RGB'))
